    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        let data = match id {
            0x00000001 => {
                ...
                DnsData::ZoneType(ZoneType::from_base_type(u32::from_le_bytes(data_bytes.try_into().unwrap())))
            },
            0x00000002 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::AllowUpdate(AllowUpdate::from_base_type(u32::from_le_bytes(data_bytes.try_into().unwrap())))
            },
            0x00000008 => {
                if data_bytes.len() != 8 {
                    return None;
                ...
                DnsData::SecureTime(utc_seconds_relative_to_1601(seconds))
            },
            0x00000010 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::NoRefreshInterval { hours: u32::from_le_bytes(data_bytes.try_into().unwrap()) }
            },
            0x00000020 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::RefreshInterval { hours: u32::from_le_bytes(data_bytes.try_into().unwrap()) }
            },
            0x00000040 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::AgingState(Boolean::from_base_type(u32::from_le_bytes(data_bytes.try_into().unwrap())))
            },
            0x00000011 => {
                let servers = parse_ip4_array(data_bytes)?;
                DnsData::ScavengingServers(servers)
            },
            0x00000012 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::AgingEnabledTime { hours: u32::from_le_bytes(data_bytes.try_into().unwrap()) }
            },
            0x00000080 => {
                ...
                DnsData::DeletedFromHostname(hostname)
            },
            0x00000081 => {
                let servers = parse_ip4_array(data_bytes)?;
                DnsData::MasterServers(servers)
            },
            0x00000082 => {
                let servers = parse_ip4_array(data_bytes)?;
                DnsData::AutoNsServers(servers)
            },
            0x00000083 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::DcPromoConvert(DcPromoFlag::from_base_type(u32::from_le_bytes(data_bytes.try_into().unwrap())))
            },
            0x00000090 => {
                let servers = parse_dns_addr_array(data_bytes)?;
                DnsData::ScavengingServersDa(servers)
            },
            0x00000091 => {
                let servers = parse_dns_addr_array(data_bytes)?;
                DnsData::MasterServersDa(servers)
            },
            0x00000092 => {
                let servers = parse_dns_addr_array(data_bytes)?;
                DnsData::AutoNsServersDa(servers)
            },
            0x00000100 => {
                if data_bytes.len() != 4 {
                    return None;
                }
                DnsData::DbFlags(DnsRpcNodeFlags::from(u32::from_le_bytes(data_bytes.try_into().unwrap())))
            },
            other => {
                DnsData::Other { id: other, data: data_bytes.to_vec() }
            },
        };
        Some(Self {
            name_length,
...
#[cfg(test)]
mod tests {
    use super::{DnsData, DnsIpAddr, DnsProperty, DnsValidationStatus};
    use std::net::{Ipv4Addr, Ipv6Addr};
    ...
    fn test_master_servers_da() {
        ...
        let master_servers_da = match &dns_property.data {
            DnsData::MasterServersDa(msda) => msda,
